...
class PlotFunctionsV2_0(GraphScene,Scene):
    ...
    def construct(self):
        ...
        graph_label1 = self.get_graph_label(graph3, label="\\frac{x^2 - 4x + 5}{2}", x_val=33.5, direction=UP)
        graph_label2 = self.get_graph_label(graph, label="\\frac{x^2 - 4x + 5}{2}", x_val=15, direction=UP)
        graph_label3 = self.get_graph_label(graph2, label="\\frac{x^2 - 4x + 5}{2}", x_val=15, direction=UP)
        ...
        text = VGroup(
            ...
            Text("(看成所对应的数量更好理解)", font='未署名的信'),# 13
        ).scale(0.5).set_color('#EEC591')

        ## 点（1）
        # y=(-x + 70)
        self.play(ShowCreation(graph3), ShowCreation(graph_label1))
        # self.wait(2)
        ...
        ## 点（2）
        # y=(-x + 35)
        self.play(ShowCreation(graph), ShowCreation(graph_label2))
        # self.wait(2)
