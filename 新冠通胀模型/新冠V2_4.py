...
class PlotFunctionsV2_4(GraphScene, Scene):
    CONFIG = {
        "x_min": 0,
        "x_max": 55,
        "x_tick_frequency": 5,
    ...
    def construct(self):
        ...
        text = VGroup(
            ...
            Text("当黑天鹅新冠肺炎造成供需双杀", font='未署名的信'),  # 22
            Text("当过多的货币追逐有限的商品", font='未署名的信'),  # 23
            Text("一个超级通胀是否正在酝酿当中", font='未署名的信'),  # 24
            Text("接下来我们将从供需的角度", font='未署名的信'),  # 25
            Text("来讨论这一问题", font='未署名的信'),  # 26

            Text("提示：该视频不作为投资建议", font='未署名的信'),  # 27

        ).scale(0.5).set_color('#1E90FF')

        text.arrange(
            aligned_edge = LEFT,
        )

        text[22].to_corner(RIGHT + 5 * UP)
        text[23].to_corner(RIGHT + 6 * UP)
        text[24].to_corner(RIGHT + 7 * UP)
        text[25].to_corner(RIGHT + 8 * UP)
        text[26].to_corner(RIGHT + 9 * UP)
        text[27].to_corner(RIGHT + 11 * UP)
        self.play(Write(text[22]), run_time=2.5)
        self.play(Write(text[23]), run_time=2.5)
        self.play(Write(text[24]), run_time=2.5)
        self.play(Write(text[25]), run_time=2.5)
        self.play(Write(text[26]), run_time=2.5)
        self.play(Write(text[27]), run_time=2.5)
        self.wait(3)
        ...
        [
            VGroup(
                text[0][0:5].set_color('#8A2BE2'),
                text[0][5:].set_color('#8A2BE2'),
            ) for i in range(2)
        ]

        self.wait(2)

        self.play(Write(text[0]), run_time=3)
        ...
        self.play(Write(text[14]), run_time=4)
        ...
        self.play(Write(text[17]), run_time=4)
        ...
        ## 总结图片
        #image = ImageMobject("新冠通胀模型").scale(2)
